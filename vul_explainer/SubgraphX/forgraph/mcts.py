...
class MCTS():
    def __init__(self, X: torch.Tensor, edge_index: torch.Tensor, n_rollout: int,
        ...
        self.min_atoms = int(0.2*X.shape[0])
        self.c_puct = c_puct
    ...
    def mcts(self, verbose=True):
        ...
        if self.graph.number_of_nodes() > 600:
            new_rollout = int(self.n_rollout/2)
            print('new rollout: {}'.format(new_rollout))
        else:
            new_rollout = self.n_rollout
        for rollout_idx in range(new_rollout):
