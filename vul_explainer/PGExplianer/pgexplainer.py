...
class PGExplainer(nn.Module):
    def __init__(self, model, epochs: int = 100, lr: float = 0.005,
        ...
        self.ckpt_path = os.path.join('/home/DIG-main/dig/xgraph/PGExplainer-master/checkpoint/', data_args.dataset_name,
    ...
    def eval_probs(self, x: torch.Tensor, edge_index: torch.Tensor, edge_attr: torch.Tensor,
        ...
        return outputs[0][0][1].squeeze()

    def explain_edge_mask(self, x, edge_index, edge_attr, **kwargs):
        with torch.no_grad():
            prob, emb = self.get_model_output(x, edge_index, edge_attr)
            edge_mask = self.forward((x, emb, edge_index, 1.0, edge_attr), training=False)
        return edge_mask
