...
class Devign(InMemoryDataset):
    ...
    @property
    def processed_file_names(self):
        return [f'func_msr15.pt']

    def process(self):
        data_list = []
        with open('input the vulnerability graph pat to be explained!!!') as f:###
            dataset_path = f.readlines()
        i = 0
        for path in dataset_path:
            data_temp = path.strip()
            data = read_json(data_temp)
            if(data.num_nodes >= 10 and data.num_nodes <=2000):
