...
from benchmark.data.dataset_gen import BA_LRP, BA_Shape, Devign
import os, sys
import copy
import json
...
def data2dict(data):
    ...
    dict['node_features'] = x

    edge_index = data.edge_index.t().numpy().tolist()
    edge_attr = data.edge_attr.numpy().tolist()
    graph = []
    for i in range(len(edge_index)):
        graph.append([edge_index[i][0] , edge_attr[i][0] , edge_index[i][1]])
    dict['graph'] = graph

    dict['target'] = data.y.numpy().tolist()[0]

    return dict        


def create_input(dataset,save_path,model):
    dict={}
    
    for i,data in enumerate(dataset):
        dict[i] = data2dict(data)
    
    filename = os.path.join(save_path, model+'_Devigninput.json')
...
def load_dataset(name: str) -> dir:
    ...
    if name in molecule_set:
        ...
        dataset = Devign(root='/home/DIG-main/dig/xgraph/GNNExplainer-master/dataset',#root=os.path.join(ROOT_DIR, '..', 'datasets', 'devign'),
                        num_per_class=10000)
        ...
        dataset_split[1]=0
        #dataset_split[2] = dataset_len - dataset_split[0] - dataset_split[1]
        dataset_split[2] = dataset_len
        train_set, val_set, test_set = \
            random_split(dataset, dataset_split)

        #save data from dataset to json format file for the input of model
        #save_path='/home/mytest/GNNExplainer-master/dataset/'
        #create_input(train_set,save_path,'train')
        #create_input(val_set,save_path,'valid')
        #create_input(test_set,save_path,'test')
