...
class Devign(InMemoryDataset):
    ...
    def process(self):
        ...
        dataset_list = glob.glob('input the vulnerability graph path to be explained!!!')###
        random.shuffle(dataset_list)
        i = 0
        for data_name in dataset_list:
            data_name = data_name.strip()
            #data_name = '/home/GNNLRP_model/input_data_com/'+data_name
            i += 1
            #if i>7:
            #    break
            data = read_json(data_name)
            if(data.num_nodes >= 10 and data.num_nodes <= 10000):
